...
export default function MasterDashboard() {
    ...
    const handleCreateClient = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch('/api/master/tenants', {
                ...
                body: JSON.stringify({ slug: newSlug, password: newPassword, website_url: newWebsiteUrl })
            });
    ...
    const handleUpdateWebsiteUrl = async (slug: string, url: string) => {
        try {
            const res = await fetch('/api/master/tenants', {
                method: 'PATCH',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ slug, website_url: url })
            });
            if (!res.ok) throw new Error('Error al actualizar URL');
            setTenants(prev => prev.map(t => t.cliente_slug === slug ? { ...t, website_url: url } : t));
        } catch (err: any) {
            alert(err.message);
        }
    };
        ...
        <div className="flex flex-col md:flex-row h-screen bg-[#050505] text-zinc-100 font-sans overflow-hidden">
                ...
                <div className="px-4 py-8 flex-1">
                    <nav className="space-y-1">
                        <button
                            ...
                            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-xs uppercase tracking-widest transition-all ${activeTab === 'overview' ? 'bg-white/5 text-white border border-white/10' : 'hover:bg-white/5 text-zinc-500 hover:text-white'}`}>
                            <LayoutDashboard className="w-4 h-4" />
                            Overview
                        </button>
                        <button
                            onClick={() => setActiveTab('settings')}
                            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold text-xs uppercase tracking-widest transition-all ${activeTab === 'settings' ? 'bg-white/5 text-white border border-white/10' : 'hover:bg-white/5 text-zinc-500 hover:text-white'}`}>
            ...
            <main className="flex-1 overflow-y-auto overflow-x-hidden w-full relative h-[calc(100vh-64px)] md:h-screen">
                ...
                {activeTab === 'overview' ? (
                                                ...
                                                <div className="flex items-center gap-6">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="flex flex-col gap-2 mt-2">
                                                            ...
                                                            <div className="flex items-center gap-2 group/url relative">
                                                                <Globe className="w-3.5 h-3.5 text-zinc-700 transition-colors group-hover/url:text-white" />
                                                                <input
                                                                    type="text"
                                                                    placeholder="Link de la página (ej: https://...)"
                                                                    defaultValue={tenant.website_url}
                                                                    onBlur={(e) => handleUpdateWebsiteUrl(tenant.cliente_slug, e.target.value)}
                                                                    className="bg-transparent border-none outline-none text-[11px] text-zinc-500 font-bold placeholder-zinc-800 w-64 hover:text-white focus:text-white transition-all lowercase"
                                                                />
                                                                {tenant.website_url && (
                                                                    <a
                                                                        href={tenant.website_url.startsWith('http') ? tenant.website_url : `https://${tenant.website_url}`}
                                                                        target="_blank"
                                                                        rel="noopener noreferrer"
                                                                        className="p-1.5 bg-white/5 rounded-lg text-zinc-600 hover:text-white hover:bg-white/10 transition-all opacity-0 group-hover/url:opacity-100"
                                                                    >
                    ...
                    <div className="max-w-4xl mx-auto p-12 lg:p-24 relative z-10">
                        <div className="flex items-center gap-4 mb-16">
                            <div className="w-16 h-16 rounded-[2rem] bg-white/5 border border-white/10 flex items-center justify-center text-zinc-400">
                                <Settings className="w-8 h-8" />
                            </div>
                            <div>
                                <h2 className="text-4xl md:text-5xl font-black text-white italic uppercase tracking-tighter">Ajustes del <span className="text-zinc-600">Sistema</span></h2>
                                <p className="text-[10px] text-zinc-500 font-black tracking-[0.4em] uppercase mt-1">Configuración Global Vyte Core</p>
                            </div>
                        </div>

                        <div className="grid gap-8">
                            <div className="p-10 bg-[#0a0a0a] border border-white/5 rounded-[3rem] shadow-2xl relative overflow-hidden group">
                                <div className="absolute top-0 right-0 w-32 h-32 bg-white/[0.01] rounded-full blur-3xl pointer-events-none -translate-y-1/2 translate-x-1/2"></div>
                                <h4 className="text-[11px] font-black uppercase tracking-[0.4em] text-zinc-600 mb-8 border-b border-white/5 pb-6">Estado del Servidor (Real-time)</h4>
                                <div className="space-y-6">
                                    <div className="flex justify-between items-center group/item hover:bg-white/[0.02] p-4 rounded-2xl transition-all">
                                        <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Protocolo Core</span>
                                        <span className="text-xs font-black text-white px-3 py-1 bg-white/5 rounded-full border border-white/10 tracking-widest italic">V1.7.2-ALPHA</span>
                                    </div>
                                    <div className="flex justify-between items-center group/item hover:bg-white/[0.02] p-4 rounded-2xl transition-all">
                                        <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Base de Datos</span>
                                        <span className="text-xs font-black text-zinc-400 uppercase tracking-widest italic">Hostinger MySQL Cloud</span>
                                    </div>
                                    <div className="flex justify-between items-center group/item hover:bg-white/[0.02] p-4 rounded-2xl transition-all">
                                        <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Latencia Global</span>
                                        <div className="flex items-center gap-3">
                                            <div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></div>
                                            <span className="text-xs font-black text-green-500 uppercase tracking-widest italic animate-pulse">Optima (14ms)</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="fixed inset-0 bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 md:p-8 z-[100]">
                    <div className="bg-[#050505] border border-white/10 rounded-[2.5rem] md:rounded-[3rem] p-8 md:p-16 max-w-xl w-full shadow-[0_50px_100px_rgba(0,0,0,0.5)] relative overflow-y-auto max-h-[90vh] custom-scrollbar">
                        ...
                        <form onSubmit={handleCreateClient} className="space-y-8 relative z-10">
                            ...
                            <div>
                                <label className="block text-[10px] font-black tracking-[0.3em] uppercase text-zinc-500 mb-4 ml-6">URL del Sitio (Opcional)</label>
                                <input
                                    type="text"
